...
When you run run_analysis.R the script will download the dataset, read in the apropriate files/values and create a tidy full dataset. The script will then summarise and create another data set with the average of each value for each activity and each subject, then write this out to tidy_second_summary.txt

The result is the found in tidy_second_summary.txt

Below follows a Code Book describing the variables
